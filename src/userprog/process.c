...
start_process (void *file_name_)
{
  ...
  if (success)
  {
      ...
      list_push_back(&parent->child_processes_list,&child->child_elem);
...
process_wait (tid_t child_tid UNUSED) 
{
    ...
    for (struct list_elem* e = list_begin (&parent->child_processes_list); e != list_end (&parent->child_processes_list);
         e = list_next (e))
    {
...
process_exit (void)
{
  ...
  uint32_t *pd;
    ...
    while (!list_empty(&cur->opened_files_list))
    {
        struct open_file* opened_file = list_entry(list_pop_back(&cur->opened_files_list), struct open_file, elem);
        file_close(opened_file->ptr);
        palloc_free_page(opened_file);
    }

    while (!list_empty(&cur->child_processes_list))
    {
        struct thread* child = list_entry(list_pop_back(&cur->child_processes_list), struct thread, child_elem);
        child->parent= NULL;
        sema_up(&child->parent_child_sync);
    }
